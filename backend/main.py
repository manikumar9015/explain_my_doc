...
app = FastAPI(
    ...
    description="API for session-based document analysis with memory.",
    version="3.2.0-memory",
)
...
# Pydantic models updated for chat history
...
async def process_document(file: UploadFile = File(...)):
    # This endpoint's code remains unchanged from the previous version
    try:
    ...
    content_type = file.content_type; filename = file.filename; extracted_text = ""
    ...
    else: vector_store_instance.delete_collection(session_id); raise HTTPException(status_code=400, detail=f"Unsupported file type: '{content_type}'.")
    ...
    if not extracted_text.strip(): vector_store_instance.delete_collection(session_id); raise HTTPException(status_code=400, detail="Document is empty or contains no readable text.")
    ...
    try:
        ...
        vector_store_instance.add_documents(collection_name=session_id, chunks=text_chunks, embeddings=chunk_embeddings, metadatas=metadatas)
    except Exception as e: raise HTTPException(status_code=500, detail=f"Failed to store document in session {session_id}: {e}")
    return JSONResponse(status_code=200, content={"message": "Document processed successfully.", "session_id": session_id})
